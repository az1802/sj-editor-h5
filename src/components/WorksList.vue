      ...
      <a-col :span="6" v-for="item in listWithBarcode" :key="item.id" class="poster-item">
        <a-card hoverable @mouseenter="() => showBarcode(item.id, item.barcodeUrl)">
          <template v-slot:cover>
            ...
            <div class="hover-item">
              <div
                ...
                v-if="item.status === '2'"
              ></div>
              <router-link :to="`/editor/${item.id}`" v-else>
                <a-button size="large" type="primary">继续编辑该作品</a-button>
              </router-link>
            </div>
          </template>
          <template class="ant-card-actions" v-slot:actions>
            ...
            <a
              href="javascript:;"
              @click.prevent="staticClicked(item.id)"
              v-if="item.status === '2'"
            >
              ...
              <template v-slot:overlay>
                <a-menu class="overlay-dropdown">
                  ...
                  <a-menu-item v-if="item.coverImg">
                    <a href="javascript:;" @click.prevent="saveImage(item.coverImg)">
                      <DownloadOutlined />
                      下载图片
                    </a>
                  </a-menu-item>
                  <a-menu-item>
                    <a href="javascript:;" @click.prevent="sendClicked(item.id)">
                      <GiftOutlined />
                      转赠
                    </a>
                  </a-menu-item>
